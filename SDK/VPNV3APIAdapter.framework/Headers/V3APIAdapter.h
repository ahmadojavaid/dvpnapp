...
/**
 * Callback type used for events completion
 */
typedef void (^APIEventsCompletionHandler)(BOOL, NSError *);
...
typedef NS_ENUM(NSInteger, APIError) {
	...
	// Missing a required field in the payload
	APIErrorUnknownField       = 1013,
...
static NSString * const kV3AlternateUrlsKey    = @"V3AlternateUrls";
...
static NSString * const kV3ServerListFileKey   = @"serverList.json";
...
/**
 * Public getter for connection options; useful for things like accessing the fileURL of the Core Data store
 */
- (id)getOption:(NSString *)key;



- (void)sendEvent:(NSDictionary *)event withAccessToken:(NSString *)accessToken andCompletionHandler:(APIEventsCompletionHandler)completionHandler;
